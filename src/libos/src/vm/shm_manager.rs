...
pub enum MmapSharedResult {
    ...
    NeedCreate,
    /// Current shared chunk needs to expand range. (old shared chunk, addr that to be returned after expansion)
    NeedExpand(ChunkRef, usize),
            ...
            match *options.addr() {
                ...
                vm_util::VMMapAddr::Need(addr) | vm_util::VMMapAddr::Force(addr) => {
                    if addr == sc_addr && offset == sc_offset && new_size <= sc_size {
                        ...
                        return Ok(MmapSharedResult::NeedExpand(shared_chunk.clone(), addr));
                    } else if exclusived {
                        return Ok(MmapSharedResult::NeedReplace(shared_chunk.clone()));
                    } else {
                        return_errno!(EINVAL, "mmap shared chunk failed");
