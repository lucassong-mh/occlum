...
use net::socket::{CmsgHdr, Iovs, MsgHdr, MsgHdrMut};
use std::fmt;
...
impl Stream {
    ...
    pub fn recvmsg(&self, msg_hdr: &mut MsgHdrMut, flags: RecvFlags) -> Result<usize> {
        ...
        if let Some(msg_control) = msg_hdr.get_control_mut() {
            ...
            // Parse msg_control to cmsghdr, extract and reassign fd
            CmsgHdr::from_bytes(msg_control).map(|mut cmsg_hdr| {
                let send_fd = cmsg_hdr.get_fd().unwrap();
                ...
                let ipc_file = process::table::get_thread(send_tid)
                    ...
                    .get(send_fd)
                    .unwrap();
                let recv_fd = current!().add_file(ipc_file.clone(), false);
                cmsg_hdr.set_fd(recv_fd);
            });
        }
        Ok(data_len)
    }
    ...
    fn peer_ancillary(&self) -> Option<Ancillary> {
        if let Status::Connected(endpoint) = &*self.inner() {
            if let Some(ancillary) = endpoint.peer_ancillary() {
