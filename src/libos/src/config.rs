...
impl ConfigApp {
    pub fn from_user(config: &user_rootfs_config) -> Result<ConfigApp> {
        ...
        if upper_layer.is_some() {
            let layer_mount_configs = root_mount_config.options.layers.as_mut().unwrap();
            // container AsyncSFS in layers
            let root_container_async_sfs_mount_config = layer_mount_configs
                .iter_mut()
                .find(|m| {
                    m.target == Path::new("/")
                ...
                .ok_or_else(|| {
                    errno!(
                        Errno::ENOENT,
                        "the container AsyncSFS in layers is not valid"
                    )
                })?;

            root_container_async_sfs_mount_config.source = upper_layer;
        }
